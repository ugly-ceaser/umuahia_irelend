document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".modals").forEach(modal => modal.style.display = "none");
    ...
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("status")) {
            ...
            selectedMember = event.target.closest("tr").querySelector("td span").textContent.trim();
            ...
            buttons.confirmStatus.textContent = selectedStatus.classList.contains("available") ? "Yes, Deactivate" : "Yes, Make Available";
    ...
    buttons.confirmStatus.addEventListener("click", function () {
        if (selectedStatus && selectedMember) {
            let storedStatuses = JSON.parse(localStorage.getItem("memberStatuses")) || {};
            updateStatusUI(selectedStatus, selectedStatus.classList.contains("available") ? "Deactivated" : "Available");
            storedStatuses[selectedMember] = selectedStatus.classList.contains("available") ? "Available" : "Deactivated";
    ...
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("delete-user")) {
            ...
            document.getElementById("deleteMemberName").textContent = selectedUserRow.querySelector("span").textContent;
    ...
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("action-icon")) {
            ...
            document.querySelectorAll(".action-list").forEach(menu => menu.style.display = "none");
        }
    });
    ...
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("flag-star")) {
            ...
            document.getElementById("flagStarName").textContent = userRow.querySelector("span").textContent;
            document.getElementById("flagStarEmail").textContent = userRow.querySelector("td:nth-child(2)").textContent;
            showModal(modals.flagStar);
        }

        if (event.target.classList.contains("flag-icon") || event.target.classList.contains("star-icon")) {
            event.target.classList.toggle("active");
            showToast(event.target.classList.contains("active") ? `User ${event.target.classList.contains("flag-icon") ? "Flagged" : "Starred"}` : `User ${event.target.classList.contains("flag-icon") ? "Unflagged" : "Unstarred"}`);
        }
    });
    ...
    function loadActiveMenu() {
        ...
        menuItems.forEach(item => item.classList.toggle("active", item.getAttribute("data-page") === activeMenu));
    }
